...
class ImageSplitter:
    ...
    def split_img_tensor(self, img_tensor):
        ...
        Z = torch.zeros([batch, 1, height+delta, width+delta])
    ...
    def merge_img_tensor(self, list_img_tensor):
        ...
        out = torch.zeros((1, 1, new_height, new_width))
        mask = torch.zeros((1, 1, new_height, new_width))
        ...
        for i in range(0, new_height, stride):
            for j in range(0, new_width, stride):
                ...
                mask_patch = torch.zeros((1, 1, new_height, new_width))
                out_patch = torch.zeros((1, 1, new_height, new_width))
