...
class ABPN_v5(nn.Module):
    ...
    def forward(self, x):
        # feature extraction
        bic_x = F.interpolate(x, scale_factor=3, mode='bicubic')
        feat_x = self.feat1(x)
        SA0 = self.SA0(feat_x)
        feat_x = self.feat2(SA0)
        # BP 1
        ...
        LR_res = x - F.interpolate(SR, scale_factor=1/3, mode='bicubic')
        LR_res = self.final_feat1(LR_res)
        LR_SA = self.final_SA0(LR_res)
        LR_res = self.final_feat2(LR_SA)

        SR_res = F.interpolate(LR_res, scale_factor=3, mode='bicubic')
