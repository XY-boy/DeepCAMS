...
class XVFInet(nn.Module):
    ...
    def __init__(self, num_fea, scale, ):
        super(XVFInet, self).__init__()
        self.nf = num_fea  # 64
        self.scale = scale  # 4
        self.vfinet = VFInet(num_fea, scale)
        self.lrelu = nn.ReLU()
        self.in_channels = 1
        self.scale_train = self.scale_test = 1
        ...
        self.rec_ext_ds_module.append(RResBlock2D_3D(num_fea, T_reduce_flag=False))
        self.rec_ext_ds_module = nn.Sequential(*self.rec_ext_ds_module)

        self.rec_ctx_ds = nn.Conv3d(self.nf, self.nf, [1, 3, 3], [1, 2, 2], [0, 1, 1])

        print("The lowest scale depth for training (S_trn): ", self.scale_train)
        print("The lowest scale depth for test (S_tst): ", self.scale_test)
    ...
    def forward(self, x, t_value, is_training=True):
        ...
        feat_x_list = [feat_x]
        self.lowest_depth_level = self.scale_train if is_training else self.scale_test
        ...
        if is_training:
            out_l_list = []
            flow_refine_l_list = []
            out_l, flow_l, flow_refine_l = self.vfinet(x, feat_x_list[self.scale_train], flow_l, t_value,
                                                       level=self.scale_train, is_training=True)
            out_l_list.append(out_l)
            flow_refine_l_list.append(flow_refine_l)
            for level in range(self.scale_train - 1, 0,
                               -1):  ## self.args.S_trn, self.args.S_trn-1, ..., 1. level 0 is not included
                out_l, flow_l = self.vfinet(x, feat_x_list[level], flow_l, t_value, level=level, is_training=True)
                out_l_list.append(out_l)
            out_l, flow_l, flow_refine_l, occ_0_l0 = self.vfinet(x, feat_x_list[0], flow_l, t_value, level=0,
                                                                 is_training=True)
            out_l_list.append(out_l)
            flow_refine_l_list.append(flow_refine_l)
            return out_l_list[::-1], flow_refine_l_list[::-1], occ_0_l0, torch.mean(x,
                                                                                    dim=2)  # out_l_list should be reversed. [out_l0, out_l1, ...]

        else:  # Testing
            for level in range(self.scale_test, 0,
                               -1):  ## self.args.S_tst, self.args.S_tst-1, ..., 1. level 0 is not included
...
class VFInet(nn.Module):
    ...
    def __init__(self, num_fea, scale):
        ...
        self.refine_unet = RefineUNet(num_fea, scale)
        self.lrelu = nn.ReLU()

        # self.conv_warp = nn.Conv2d(self.nf + 2, self.nf, 3, 1)
    ...
    def forward(self, x, feat_x, flow_l_prev, t_value, level, is_training):
        ...
        if flow_l_prev is None:
            ...
            up_flow_l_prev = 2.0 * F.interpolate(flow_l_prev.detach(), scale_factor=(2, 2), mode='bilinear',
                                                 align_corners=False)
            warped_feat1_l = self.bwarp(feat1_l, up_flow_l_prev[:, :2, :, :])
            warped_feat0_l = self.bwarp(feat0_l, up_flow_l_prev[:, 2:, :, :])
            flow_l_tmp = self.conv_flow2(torch.cat([self.conv_flow1(torch.cat([feat0_l, warped_feat1_l], dim=1)),
                                                    self.conv_flow1(torch.cat([feat1_l, warped_feat0_l], dim=1)),
                                                    up_flow_l_prev], dim=1))
        ...
        flow_forward, norm0_l = self.z_fwarp(flow_01_l, t_value * flow_01_l,
                                             z_01_l)  ## Actually, F (t) -> (t+1). Translation only. Not normalized yet
        flow_backward, norm1_l = self.z_fwarp(flow_10_l, (1 - t_value) * flow_10_l,
                                              z_10_l)  ## Actually, F (1-t) -> (-t). Translation only. Not normalized yet

        flow_t0_l = -(1 - t_value) * ((t_value) * flow_forward) + (t_value) * (
                    (t_value) * flow_backward)  # The numerator of Eq.(1) in the paper.
        flow_t1_l = (1 - t_value) * ((1 - t_value) * flow_forward) - (t_value) * (
                    (1 - t_value) * flow_backward)  # The numerator of Eq.(2) in the paper.

        norm_l = (1 - t_value) * norm0_l + t_value * norm1_l
        mask_ = (norm_l.detach() > 0).type(norm_l.type())
        flow_t0_l = (1 - mask_) * flow_t0_l + mask_ * (flow_t0_l.clone() / (
                    norm_l.clone() + (1 - mask_)))  # Divide the numerator with denominator in Eq.(1)
        flow_t1_l = (1 - mask_) * flow_t1_l + mask_ * (flow_t1_l.clone() / (
                    norm_l.clone() + (1 - mask_)))  # Divide the numerator with denominator in Eq.(2)

        ## Feature warping  改成 直接卷试试
        ...
        ## Flow upscale
        flow_t0_l = self.scale * F.interpolate(flow_t0_l, scale_factor=(self.scale, self.scale), mode='bilinear',
                                               align_corners=False)
        flow_t1_l = self.scale * F.interpolate(flow_t1_l, scale_factor=(self.scale, self.scale), mode='bilinear',
                                               align_corners=False)
        ...
        # print(warped0_l.size())
        # print(222222 )
        warped_img1_l = self.bwarp(x_l[:, :, 1, :, :], flow_t1_l)

        refine_out = self.refine_unet(torch.cat(
            [F.pixel_shuffle(torch.cat([feat0_l, feat1_l, warped0_l, warped1_l], dim=1), self.scale),
             x_l[:, :, 0, :, :], x_l[:, :, 1, :, :], warped_img0_l, warped_img1_l, flow_t0_l, flow_t1_l], dim=1))
    ...
    def sample_one(self, img, shiftx, shifty, weight):
        ...
        flat_basex = torch.arange(0, H, requires_grad=False).view(-1, 1)[None, None].to(self.device).long().repeat(N, C,
                                                                                                                   1,
                                                                                                                   W).view(
            -1)
        flat_basey = torch.arange(0, W, requires_grad=False).view(1, -1)[None, None].to(self.device).long().repeat(N, C,
                                                                                                                   H,
                                                                                                                   1).view(
            -1)
        ...
        idxn = torch.arange(0, N, requires_grad=False).view(N, 1, 1, 1).to(self.device).long().repeat(1, C, H, W).view(
            -1)
        idxc = torch.arange(0, C, requires_grad=False).view(1, C, 1, 1).to(self.device).long().repeat(N, 1, H, W).view(
            -1)
...
class RefineUNet(nn.Module):
    def __init__(self, num_fea, scale):
        ...
        self.enc1 = nn.Conv2d((4 * self.nf) // self.scale // self.scale + 4 * self.ch + 4, self.nf, [4, 4], 2, [1, 1])
        ...
        self.dec3 = nn.Conv2d(self.nf, 1 + self.ch, [3, 3], 1, [1, 1])  ## input added with warped image
...
class RResBlock2D_3D(nn.Module):
    ...
    def __init__(self, num_fea, T_reduce_flag=False):
        super(RResBlock2D_3D, self).__init__()
        # self.args = args
        self.nf = num_fea
        self.T_reduce_flag = T_reduce_flag
        self.resblock1 = ResBlock2D_3D(self.nf)
        self.resblock2 = ResBlock2D_3D(self.nf)
        if T_reduce_flag:
