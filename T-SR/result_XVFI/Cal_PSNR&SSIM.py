...
def main():
    ...
    folder_GT = 'D:\STVSR\Experiment_results/baseline\GT-97'  # GT 图像路径（不要包含中文）
    folder_test = 'D:\STVSR\Experiment_results/baseline\ZoomingSloM-0-96'  # 所要计算的结果路径

    # subfolder_l = sorted(glob.glob(osp.join(folder_test, '*')))  # 子文件夹路径的list，即
    subfolder_l = sorted(glob.glob(osp.join(folder_GT, '*')))  # 子文件夹路径的list，即
    subfolder_GT_l = sorted(glob.glob(osp.join(folder_GT, '*')))
    print(subfolder_l)
    print(subfolder_GT_l)

    subfolder_name_l = []  # 子文件夹名，即视频的编号
    ...
    all_odd_PSNR = []  # 奇数帧，插入的帧
    all_odd_SSIM = []
    all_even_PSNR = [] # 偶数帧，原本存在的帧
    all_even_SSIM = []
    ...
    for subfolder, subfolder_GT in zip(subfolder_l, subfolder_GT_l):
        ...
        img_list = sorted(glob.glob(osp.join(subfolder_GT, '*')))  # 真值图像路径列表
        ...
        for i, img_path in enumerate(img_list):
            ...
            # im_GT = cv2.imread(os.path.join(folder_GT, subfolder_name, base_name + suffix + '.png')) / 255.
            ...
            SSIM = calculate_rmse(cropped_GT * 255, cropped_Gen * 255)
            ...
            logger.info('Folder {} {:3d} - {:25}. \tPSNR: {:.6f} dB, \tSSIM: {:.6f}'.format(subfolder_name,
                i + 1, base_name, PSNR, SSIM))
            if PSNR <100:
                PSNR_all.append(PSNR)
                SSIM_all.append(SSIM)
            if (int(base_name) % 2) == 0:  # even_frame
                even_PSNR.append(PSNR)
                even_SSIM.append(SSIM)
                print('偶数帧{}'.format(base_name))
            if (int(base_name) % 2) != 0:  # odd_frame
                odd_PSNR.append(PSNR)
                odd_SSIM.append(SSIM)
                print('奇数帧{}'.format(base_name))


        # print('Average: PSNR: {:.6f} dB, SSIM: {:.6f}'.format(
        #     sum(PSNR_all) / len(PSNR_all),
        #     sum(SSIM_all) / len(SSIM_all)))
        # logger.info('Folder {}-Average: PSNR: {:.6f} dB, SSIM: {:.6f}'.format(subfolder_name,
        #     sum(PSNR_all) / len(PSNR_all),
        #     sum(SSIM_all) / len(SSIM_all)))
        logger.info('Folder {}-Average: odd_PSNR: {:.6f} dB, odd_SSIM: {:.6f}, even_PSNR: {:.6f} dB,even_SSIM: {:.6f}'.format(subfolder_name,
                                                                              sum(odd_PSNR) / len(odd_PSNR),
                                                                              sum(odd_SSIM) / len(odd_SSIM),
                    sum(even_PSNR)/len(even_PSNR),sum(even_SSIM)/len(even_SSIM)))
        All_test_PSNR.append(sum(PSNR_all) / len(PSNR_all))
        All_test_SSIM.append(sum(SSIM_all) / len(SSIM_all))
        all_even_PSNR.append(sum(even_PSNR)/len(even_PSNR))
        all_even_SSIM.append(sum(even_SSIM)/len(even_SSIM))
        all_odd_PSNR.append(sum(odd_PSNR) / len(odd_PSNR))
        all_odd_SSIM.append(sum(odd_SSIM) / len(odd_SSIM))
    # print(All_test_PSNR)
    # print(all_even_PSNR)

        logger.info('**********************************Final*************************************')
        logger.info('All Folder Average: PSNR: {:.6f} dB, SSIM: {:.6f}'.format(sum(All_test_PSNR) / len(All_test_PSNR), sum(All_test_SSIM) / len(All_test_SSIM)))
        logger.info('All Folder Average: odd_PSNR: {:.6f} dB, odd_SSIM: {:.6f}'.format(sum(all_odd_PSNR) / len(all_odd_PSNR), sum(all_odd_SSIM) / len(all_odd_SSIM)))
        logger.info('All Folder Average: even_PSNR: {:.6f} dB, even_SSIM: {:.6f}'.format(sum(all_even_PSNR) / len(all_even_PSNR), sum(all_even_SSIM) / len(all_even_SSIM)))
